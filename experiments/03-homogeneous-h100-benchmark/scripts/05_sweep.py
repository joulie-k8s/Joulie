...
def main():
    ...
    kind_cfg_path = pathlib.Path(get_cfg(cfg, "install", "kind_cluster_config", default="experiments/03-homogeneous-h100-benchmark/configs/kind-cluster.yaml"))
    if kind_cfg_path.exists():
        kind_cfg = yaml.safe_load(kind_cfg_path.read_text()) or {}
        install_env_base["CLUSTER_NAME"] = kind_cfg.get("name", "joulie-homogeneous-h100")
