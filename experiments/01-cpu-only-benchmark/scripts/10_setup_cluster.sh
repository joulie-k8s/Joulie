...
./experiments/01-cpu-only-benchmark/scripts/00_prereqs_check.sh
printf '\n2) CREATING CLUSTER\n\n'
./experiments/01-cpu-only-benchmark/scripts/01_create_cluster_kwokctl.sh "$CFG"
printf '\n3) GENERATING AND APPLYING CPU-ONLY KWOK NODES\n\n'
./experiments/01-cpu-only-benchmark/scripts/02_apply_nodes.sh "$INVENTORY"
printf '\nSetup completed.\n'
