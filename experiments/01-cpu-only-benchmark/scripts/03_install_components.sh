...
BASELINE=${1:-B}
JOULIE_REGISTRY=${JOULIE_REGISTRY:-registry.cern.ch/joulie}
JOULIE_TAG=${JOULIE_TAG:-latest}
SIM_REGISTRY=${SIM_REGISTRY:-registry.cern.ch/joulie}
...
PERFORMANCE_CAP_WATTS=${PERFORMANCE_CAP_WATTS:-5000}
ECO_CAP_WATTS=${ECO_CAP_WATTS:-120}
...
if [[ "$BASELINE" == "A" ]]; then
  ...
  INFRA_NODE=${INFRA_NODE:-${CLUSTER_NAME:-joulie-cpu-benchmark}-worker}
  ...
  CTRL_CONTAINER="${CLUSTER_NAME:-joulie-cpu-benchmark}-control-plane"
  if docker inspect "$CTRL_CONTAINER" >/dev/null 2>&1; then
    # KinD: check from control-plane node
    ...
    # Real cluster: probe via port-forward
