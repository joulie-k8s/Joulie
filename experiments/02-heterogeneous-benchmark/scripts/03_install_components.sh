...
if [[ "$BASELINE" == "A" ]]; then
  ...
  helm upgrade --install joulie "$ROOT/charts/joulie" -n joulie-system --create-namespace \
    ...
    --set "controllerManager.env.GPU_PERFORMANCE_CAP_PCT_OF_MAX=${GPU_PERFORMANCE_CAP_PCT_OF_MAX}" \
    --set "controllerManager.env.GPU_ECO_CAP_PCT_OF_MAX=${GPU_ECO_CAP_PCT_OF_MAX}" \
    --set "controllerManager.env.GPU_WRITE_ABSOLUTE_CAPS=${GPU_WRITE_ABSOLUTE_CAPS}" \
