...
This example shows how to configure the central controller manager policy loop without changing code.

## What can be configured

The controller manager reads these env vars:
...
## 2) Apply a custom controller manager config

Use the provided patch file:

```bash
kubectl -n joulie-system patch deployment joulie-controller-manager --type merge \
  --patch-file examples/04-operator-configuration/controller-manager-env-patch.yaml
kubectl -n joulie-system rollout status deployment/joulie-controller-manager
```
...
- `Policy Reassignment Events`: shows how often controller manager is changing node profile.
...
```bash
kubectl -n joulie-system set env deploy/joulie-controller-manager \
...
- `NODE_SELECTOR` controls which nodes the controller manager manages. It does **not** control DaemonSet placement.
